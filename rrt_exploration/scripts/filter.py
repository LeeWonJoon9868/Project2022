...
from functions import gridValue,informationGain,informationGain2
...
def callBack(data,args):
	...
	if len(frontiers)>0:
		...
		frontiers=x
def initBack(data,args):
	global node_init
	transformedPoint=args[0].transformPoint(args[1],data)
	q=[array([transformedPoint.point.x,transformedPoint.point.y])]
	node_init=q

	rospy.loginfo(" init_node "+str(node_init))
...
def node():
	...
	init_topic= rospy.get_param('~init_topic','/init_points')	
...
#wait if any of robots' global costmap map is not received yet
	...
	rospy.Subscriber(init_topic, PointStamped, callback=initBack,callback_args=[tfLisn,global_frame[1:]])
	pub = rospy.Publisher('frontiers', Marker, queue_size=10)
	pub2 = rospy.Publisher('centroids', Marker, queue_size=10)
	pub3 = rospy.Publisher('deleted', PointStamped, queue_size=1)
...
#clearing old frontiers  
		...
		z=0
		while (z<len(centroids) and 0<len(node_init)):
			cond=False
			temppoint.point.x=centroids[z][0]
			temppoint.point.y=centroids[z][1]
			dd=PointStamped()
			dd.header.stamp= rospy.Time.now()
			dd.header.frame_id= mapData.header.frame_id			
			for i in range(0,n_robots):
			...
			rospy.loginfo("dest"+str(centroids[z][0])+","+str(centroids[z][1]) + ","+str(node_init[0][0]) +"," +str(node_init[0][1]))
			if (norm(node_init[0]-centroids[z])<0.35):

				centroids=delete(centroids, (z), axis=0)
				frontiers=delete(frontiers, (z), axis=0)
				rospy.loginfo("s"+str(centroids))
				z=z-1
			
		#	elif (informationGain2(mapData,[centroids[z][0],centroids[z][1]],info_radius*0.3)>1):
#
		#		dd.point.x=centroids[z][0]
#				dd.point.y=centroids[z][1]
#				dd.point.z=0.0
#
#				pub3.publish(dd)
#				rospy.loginfo("del"+str(centroids[z][0])+","+str(centroids[z][1]))
#				centroids=delete(centroids, (z), axis=0)
#				frontiers=delete(frontiers, (z), axis=0)
#
#				z=z-1
